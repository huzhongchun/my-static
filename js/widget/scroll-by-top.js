 ...
 */
    ...
    scrollObject.prototype = {
        ...
        init: function() {
            ...
            self.dom = $('#'+opt.id).css({'position':'absolute','top':'0','left':'0'});
                ...
                if (p.currentY <= opt.startPullDownMaxLength && p.targetY > opt.startPullDownMaxLength && self.maxMove > 0) {
                    ...
                    if(self.maxMove > 0){
                        if (p.targetY < -(self.maxMove + opt.startPullDownMaxLength))
                            $content.css('top', (-(opt.startPullDownMaxLength + self.maxMove)) + 'px');
                        else
                            $content.css('top',  p.targetY + 'px');
                    }
                }
            });
        ...
        getTranslateArray: function() {
            ...
            var translateCss = parseInt(window.getComputedStyle( $content[0])['top']);
            if(translateCss[0] === 'none')
                translateArray = ['0','0','0']
            else
                translateArray = ['0',translateCss,'0']
        ...
        stopMoveByCss: function(currentY) {
            var self = this;
            var $content = $(this.dom);
            $content.css({'-webkit-transition': 'top 0s ease-out 0s', 'transition': 'top 0s ease-out 0s'});
            $content.css('top', currentY + 'px');
        },
        moveByCss: function(time, target) {
            var self = this, opt = this.settings;
            var $content = $(this.dom);
            $content.css({'-webkit-transition': 'top ' + time + 's cubic-bezier(0.333333,0.666667,0.66667,1) 0s', 'transition': 'top ' + time + 's cubic-bezier(0.333333,0.666667,0.66667,1) 0s'});
        ...
        monitor: function() {
            var self = this, opt = this.settings;
            var $content = $(this.dom);
            var callbackFn;(function(window){/**
 * @name 滚动条组件
 * 
 * @param {string} id 调用的元素
 * ------ {function} pullDownCallback   到顶部后继续往下滑动的回调函数，实现下拉刷新等功能
