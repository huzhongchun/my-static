...
(function(window){
    /**
    * 最好给图片加一个容器，设置高度，否则图片的加载会造成页面长度的变化，导致加载判断条件不准确，尤其对纯图片挨着布局的页面影响较大
     * @name imglazyload
    ...
    $.fn.imglazyload = function (options) {
        ...
        function isInViewport(offset) {      //图片出现在可视区的条件
            var viewOffset = isWindow ? window : viewPort.offset(),
                ...
                //console.log(viewTop+'_'+viewHeight+'_'+offset[OFFSET.img[0]]+'_'+offset[OFFSET.img[0]] + offset[OFFSET.img[1]]);
        ...
        function _load(argument) {     //加载图片，并派生事件
            var div = $(argument),
                attrObj = {},
                img = div;
...
//                div.trigger(errorEvent);
        ...
        function _detect() {     //检测图片是否出现在可视区，并对满足条件的开始加载
            ...
            for (i = pedding.length; i--;) {
                image = $(div = pedding[i]);
                offset = image.offset();
                $(div).css({'opacity':0,'-webkit-transition':'all 400ms ease'});
